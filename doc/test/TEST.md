...
| Level | Where | What it proves | Runs in |
|---|---|---|---|
| **Unit** | `test/smoke/nfs_export_lib.bats` | Test mode (`NFS_EXPORT_TEST_ROOT`) announces itself and rewires exportfs / etab / export dir; production ignores `EXPORTFS_BIN` / `NFS_ETAB` / `L4T_EXPORT_DIR` / `NFS_EXPORT_CLIENT` / `NFS_EXPORT_OPTS` from the environment, never executes a caller-supplied binary and only sudo's the literal `/usr/sbin/exportfs`, refuses a tree outside `/srv/jetson_l4t`; canonical confinement: a `..` tree, a symlinked tree, one of the three paths a symlink out, tmp's parent a symlink out (no mkdir), `nfs_export_off` on an outside / `..` tree, etab entries escaping — all refused with no `exportfs -u/-o` / mkdir; a gone tree under the export dir and a symlinked export dir are accepted; TOCTOU: the check runs right before each `-u` and each `-o` (spec rebuilt after the check) — a path swapped for an outside symlink by the exportfs stub at its `-u` gets no `-o` and the run exits 1, one swapped before its `-u` gets neither, a normal tree still gets three `-o`; the three paths; per path `exportfs -u` → `-o rw,nohide,insecure,no_subtree_check,async,no_root_squash "[fc00:1:1::/48]:<path>"`, then one `exportfs -f`, no sudo in test mode; a failed unexport does not stop the export; re-run idempotent; no `exportfs` → message + exit 0, nothing touched; missing `rootfs` / `images` → `emit_error host-config`, exit 1, nothing exported; `tools/kernel_flash/tmp` created when missing, and its creation failing → exit 1; the 1st / 2nd / 3rd `exportfs -o` failing → the rest and the flush still run, exit 1; the flush failing → exit 1; `nfs_export_off` unexports the three + flushes, ignores "not exported", works on a deleted tree, flush failure → exit 1; `nfs_export_off_all` also unexports every etab entry under the export dir (no marker), deduplicated, one flush, paths with spaces intact; `nfs_host_mountd_running` via `pgrep -x`; status reads `/var/lib/nfs/etab` (tab separated, bare client as nfs-utils writes it, `[v6]` normalised), ok only when all three are exported to the client **with rw**, warn on missing / other client / ro / unreadable table; the single `.prepared.yaml` maps to the `/srv` path (0 → 1, 2 → 2). | `devel-test` stage |
| **Integration** | `test/smoke/host_setup.bats`, `host_teardown.bats`, `jetson_cli.bats` | `host_setup.sh` exports after the `/srv` bridge with the host paths; skips with a message when nothing is prepared, when the tree is marker-only (`clean.sh build`), or when two trees are prepared (hint, run continues); outside test mode an `EXPORTFS_BIN` from the environment is never run. `host_teardown.sh` unexports *before* the bridge umount, also what the export table still lists under `/srv/jetson_l4t` when the marker is gone, and a failed flush is reported (umount still attempted, exit 1). `./jetson flash` re-exports after `sudo -v` and before `nm_flash_guard`, and stops before the guard when the export fails; `./jetson status` ⚠ on host `rpc.mountd` + a real-format etab missing a path, ✔ when all three are rw to the client, quiet without a mountd, never sudo, never runs exportfs. Suites run the lib in `NFS_EXPORT_TEST_ROOT` mode (`<tmpdir>/bin/exportfs`, no sudo); `pgrep` / `sudo` stubbed on PATH. | `devel-test` stage |
