...
- **host 自己有跑 NFS server**(裝了 `nfs-kernel-server`、`rpc.mountd` 在跑):flash 容器會透過共用的 kernel `nfsd` export L4T tree,但回答 kernel 的是 *host 的* `rpc.mountd`,拿的是 host 那份(空的)`/etc/exports`,板子的 `mount.nfs` 就在 `SSH ready` 之後一直卡住([#101](https://github.com/ycpss91255-docker/jetson_sdk_manager/issues/101))。所以 `./jetson flash`(以及 `host_setup.sh`)也會從 host 把 `rootfs`、`tools/kernel_flash/images`、`tools/kernel_flash/tmp` export 給 `fc00:1:1::/48`,選項用 NVIDIA 的 `rw,nohide,insecure,no_subtree_check,async,no_root_squash`——每次燒錄前都重做一次,重新 prepare 過的 tree 才不會拿到 stale file handle;`host_teardown.sh` 會 unexport。host 的 `rpc.mountd` 在跑卻沒有這些 export 時,`./jetson status` 會顯示 ⚠。host 沒裝 `nfs-kernel-server`?什麼都不變——容器自己的 NFS server 就夠了。
