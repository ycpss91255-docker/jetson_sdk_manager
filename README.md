...
- **Hosts that run their own NFS server** (`nfs-kernel-server` installed, `rpc.mountd` running): the flash container exports the L4T tree over the shared kernel `nfsd`, but the *host's* `rpc.mountd` answers the kernel with the host's (empty) `/etc/exports` and the board's `mount.nfs` hangs after `SSH ready` ([#101](https://github.com/ycpss91255-docker/jetson_sdk_manager/issues/101)). `./jetson flash` (and `host_setup.sh`) therefore also export `rootfs`, `tools/kernel_flash/images` and `tools/kernel_flash/tmp` from the host, to `fc00:1:1::/48`, with NVIDIA's `rw,nohide,insecure,no_subtree_check,async,no_root_squash` — re-done before every flash so a re-prepared tree never serves a stale file handle; `host_teardown.sh` unexports. `./jetson status` shows ⚠ when a host `rpc.mountd` runs without those exports. No `nfs-kernel-server` on the host? Nothing changes — the container's own NFS server is enough.
