...
- **Per boot**: `./jetson prepare` re-runs `host_setup.sh` (QEMU binfmt, `nfsd`, USB autosuspend / buffer, the `/srv/jetson_l4t` bridge, the data store mount, the host NFS export below). Nothing persists across reboots; `./jetson status` tells you when it is needed again.
- **Hosts with `nfs-kernel-server` installed** (`/usr/sbin/exportfs` exists): when the host's own `rpc.mountd` is running, it answers the shared kernel `nfsd` with the host's (empty) `/etc/exports` instead of the flash container's export, and the board's `mount.nfs` hangs after `SSH ready` ([#101](https://github.com/ycpss91255-docker/jetson_sdk_manager/issues/101)). So **whenever the host has `exportfs`** — not only while its mountd happens to be running — `./jetson flash` (and `host_setup.sh` once a tree is prepared) also export `rootfs`, `tools/kernel_flash/images` and `tools/kernel_flash/tmp` from the host, to `fc00:1:1::/48`, with NVIDIA's `rw,nohide,insecure,no_subtree_check,async,no_root_squash`; re-done before every flash so a re-prepared tree never serves a stale file handle; `host_teardown.sh` unexports (everything under `/srv/jetson_l4t`). `./jetson status` shows ⚠ when a host `rpc.mountd` runs and the exports are missing, to another client, or not `rw`. The paths, binary and options are fixed constants — nothing sudo runs comes from your environment. No `nfs-kernel-server` on the host? Nothing changes — the container's own NFS server is enough.
...
- **USB 3 ports**: the flash initrd only needs USB 2, but its gadget also tries a SuperSpeed link, and on some hosts that link never trains and its retries tear the working link down every few seconds (`Waiting for target to boot-up...` until timeout). `./jetson flash` runs `usb_ss_guard.sh auto` for you: it disables the SuperSpeed half of the Jetson's connector (sysfs, boot-scoped) and a root watcher re-enables it when the board boots; if that watcher could not be started, `./script/usb_ss_guard.sh enable`, `./jetson teardown` or a reboot restores it. Details + the manual fallback: [doc/TROUBLESHOOTING.md](doc/TROUBLESHOOTING.md#flash-waits-in-waiting-for-target-to-boot-up-while-dmesg-loops-cannot-enable-maybe-the-usb-cable-is-bad).
...
- *SSH ready*, then nothing / *Either the device cannot mount the NFS server on the host* (host runs its own `rpc.mountd`) · `mount.nfs: Stale file handle` · *Read-only file system*
